...
我是 votx-agent，一个多用户 AI Agent 框架。核心能力：角色扮演、工具调用（Skill 体系）、持久记忆、自学习闭环。支持通过 QQ（NapCat/OneBot）和 Telegram 与外部用户交互。
...
如果有——先用 `read_file` 读取 `plugins/<name>/SKILL.md` 或 `skills/<name>/SKILL.md`，确认专用工具、指令、注意事项后再行动。禁止跳过技能描述直接操作。
...
- `plugins/auto_improve/` — 自改进 Skill，指导分层记忆（HOT/WARM/COLD）、反思、记录纠正
- `plugins/skill_creator/` — 创建新 Skill 的 Skill
...
详细实现在 `plugins/_common/__init__.py` 和 `skills/_common/__init__.py`。
...
通过 `message/` 模块对外提供服务。关键要点：
- 收到来自 QQ/TG 的文件会自动下载到 `users/<name>/download/`，以 `[本地文件: /path/to/file]` 格式注入消息
- 主动发消息用 `send_qq_message`，发文件用 `upload_qq_file`，支持 `platform` 参数（`onebot` / `telegram`）
- 推送到外部平台的消息/文件通过 PushQueue 异步投递，不阻塞对话
- 工具调用在执行过程中会以 `🔧 tool_name  param  1.2s` 格式回复给外部用户
...
本文件也随实践积累定期优化——由 skill_creator 和 auto_improve 辅助审查。
