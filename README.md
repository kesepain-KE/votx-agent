...
```bash
votx        # 启动 Web UI → http://localhost:14780
```
...
脚本会安装 Python 依赖、检查并安装 `pyinstaller`、构建前端并执行打包。完成后生成 `dist\votx-agent-windows.zip`，解压后双击 `votx-agent.exe` 即可运行 Web UI（也可通过命令行附加 `--port=14780` 等参数启动）。
...
votx              # Web UI → http://localhost:14780
votx web          # 同上
votx cli          # 终端对话模式
votx help         # 查看帮助
...
# 手动 / Windows
python start_web.py              # Web UI → http://localhost:14780
python start_web.py --port=8080  # 自定义端口（冲突自动轮询）
python start.py                  # CLI 模式
```

Web UI 打开浏览器访问 `http://localhost:14780`，左侧选择用户即可开始对话。
...
│
├── skills/                     # 30 个 Skill（15 工具型 + 15 指令型）
...
| 类别 | 数量 | 说明 |
|---|---|---|
| 工具型 Skill | 15 个 | 注册 function calling：文件读写、HTTP 请求、Shell 执行、时间、Word 文档、视频下载、联网搜索、热榜查询、长期记忆、知识图谱、通用识图、Markdown 转换、任务计划、QQ/TG 消息发送、QQ/TG 文件上传 |
