...
- **多 Provider**：Kemo LLM Adapter 本地多模态网关，统一路由所有模型和能力。
...
```text
Kemo LLM Adapter — 本地多模态网关
可选模型: stepfun-step-3.7-flash, deepseek-v4-flash 等
```

配置 Base URL 和 API Key 后即可使用，可手动输入其他模型名。
...
| `image_edit` | 图像编辑，默认输出到 `users/<name>/download/` |
| `speech_generate` | 文生语音，默认输出到 `users/<name>/download/` |
| `speech_to_speech` | 语音生语音，默认输出到 `users/<name>/download/` |
| `video_generate` / `video_status` / `video_download` | 视频生成任务、查询和下载 |
...
├── plugins/            # 内置技能
├── provider/           # Kemo LLM Adapter 本地网关适配层
├── run/                # 对话引擎、历史、工具调度
├── skills/             # 用户拓展技能
├── users/              # 用户数据
