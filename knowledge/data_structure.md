...
| `users-config.md` | 用户配置文件说明，覆盖目录结构、用户头像、`config.json` 完整字段（含 `provider.timeout`/`api_style`/`vision_model`/`max_tokens`/`thinking`）、多模态能力、技能禁用、`self_soul.md`、历史记录自定义文件名 |
