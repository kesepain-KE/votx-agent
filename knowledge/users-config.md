...
Provider 接入方式：
- 满血模式 → 搭配 [VOTX LLM Adapter](https://github.com/kesepain-KE/llm-adapter-votx) 网关，多模态全开。
- 残血模式 → 直连任意 OpenAI 兼容 API（`base_url` 指向谁就是谁），部分端点（如图生图、视频、部分 ASR 路由）可能不可用。
配置上改 `base_url` 和 `api_key` 即可切换，provider type 统一填 `"votx"`。

```text
votx       统一 provider 类型；通过 base_url/api_key 切换到 VOTX 网关或 OpenAI 兼容接口
```

`python set_user.py add` 创建用户时会先显示 VOTX 配置入口，但底层仍是同一套 `type = votx + base_url/api_key`：

- Base URL（VOTX 网关或 OpenAI 兼容接口地址）
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `type` | string | `"votx"` | 服务商类型：统一写 `"votx"` |
| `model` | string | 必填 | 模型名称 |
| `api_key` | string | `""` | API 密钥，留空读取环境变量 |
| `base_url` | string | `http://127.0.0.1:8741/v1` | VOTX 网关或 OpenAI 兼容接口地址 |
...
VOTX / OpenAI-compatible 兜底：

```text
api_key:  config.json provider.api_key > VOTX_API_KEY
base_url: config.json provider.base_url > VOTX_BASE_URL > http://127.0.0.1:8741/v1
```

Provider 类型保持 `votx`；常规配置直接写入用户 `config.json`，不再推荐使用 `VOTX_PROVIDER` 覆盖。
