...
NapCat 返回的附件可能是容器内路径（如 `/app/.config/QQ/NapCat/temp/xxx`），message 模块会自动通过 Docker volume 映射将其转为宿主机路径，复制到 `users/<name>/history/file/`，并写入 `external_attachments.jsonl` 日志。无需额外配置。
