---
name: task_time
description: 管理 cron 定时任务 — 创建/查看/修改/删除每日或单次定时任务，由后台调度器自动执行
---

# 定时任务管理 (cron)

你可以帮用户管理 cron 定时任务系统。cron 是一个后台调度器，会在指定时间自动执行任务。
...
- 任务的 `command` 字段是发送给 cron 的 prompt，cron 会用 `start.py --once` 执行
