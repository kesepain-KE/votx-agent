...
from plugins._common import err, truncate, get_current_user_dir, get_effective_tool_timeout
...
def _normalize_timestamp(value) -> float:
    ...
    return timestamp if timestamp > 0 else _utc_timestamp()
...
def _resolve_call_working_dir(working_dir: str = "", default_to_user_dir: bool = True) -> tuple[str | None, str | None]:
...
def _resolve_cd_target(path_text: str, current_dir: str | None, allow_empty: bool = True) -> tuple[str | None, str | None]:
...
def _prune_session_store(store: dict, keep_session_id: str | None = None) -> list[str]:
    ...
    for _, sid_text in candidates[:overflow]:
        sessions.pop(sid_text, None)
        removed.append(sid_text)
    return removed


def _get_session_state(user_dir: str | None, session_id: str, cwd_hint: str | None = None, reset_session: bool = False) -> dict:
...
def _run_command_segment(segment: str, cwd: str | None, timeout: int, stdin_text: str, env: dict[str, str]) -> tuple[str, bool]:
    try:
...
def _format_session_env(env: dict[str, str]) -> str:
    if not env:
        return "(empty)"
    return "\n".join(f"{k}={v}" for k, v in sorted(env.items(), key=lambda item: item[0].lower()))
...
def _handle_shell_builtin(segment: str, state: dict, *, allow_history: bool = False) -> tuple[bool, str, bool]:
    try:
        tokens = _split_shell_words(segment)
    except ValueError as e:
    ...
    if head in {"unset", "unsetenv"}:
        ...
        for name in args:
            ...
            if key in env_state:
                ...
                removed.append(key)
        return True, f"OK: {', '.join(removed)}" if removed else "(empty)", True
    if head == "history":
...
def _run_shell_chain(command: str, state: dict, timeout: int, stdin_text: str, call_env: dict | None = None, *, allow_history: bool = False) -> str:
    try:
    ...
    for op, segment in parts:
        ...
        if op == "||" and last_ok:
            continue
        recognized, builtin_output, builtin_ok = _handle_shell_builtin(segment, state, allow_history=allow_history)
        ...
        output, ok = _run_command_segment(segment, current_dir, timeout, pending_stdin, env)
        pending_stdin = ""
        if output:
            outputs.append(output)
        last_ok = ok
    result = "\n".join(item for item in outputs if item)
    return truncate(result or current_dir, max_len=0)


def run_command(command: str, working_dir: str = "", timeout: int = 0, stdin: str = "", env: dict | None = None, session_id: str = "", reset_session: bool = False) -> str:
    """执行系统命令"""
    ...
    try:
        ...
        resolved_cwd, cwd_err = _resolve_call_working_dir(working_dir, default_to_user_dir=not bool(session_key))
        if cwd_err:
            return err(cwd_err)
    except ValueError as e:
        return err(str(e))
    ...
    if session_key:
        ...
        result = _run_shell_chain(cmd, state, timeout_value, stdin_text, call_env, allow_history=True)
    ...
    state = {"version": _SESSION_STATE_VERSION, "cwd": resolved_cwd or get_current_user_dir() or os.getcwd(), "previous_cwd": "", "env": {}, "history": []}
    return _run_shell_chain(cmd, state, timeout_value, stdin_text, call_env, allow_history=False)
...
SCHEMA = {
    ...
    "function": {
        "name": "run_command",
        "description": "执行系统命令。subprocess 安全模式。支持 working_dir、stdin、env、timeout 和 session_id。会话模式下 cwd/env/history 跨调用保留，可用 cd/chdir/pwd/export/unset/history，并支持 &&/||/; 命令链。超时遵循 tool.tool_timeout（用户配置 > 全局配置 > 内置默认）。Windows 下自动处理编码。当需要运行系统命令、检查环境、诊断问题、管理进程、查看文件权限、执行脚本、git 操作、系统管理时使用。仅供无专用工具的场景使用，有专用工具时优先用专用工具。",
        "parameters": {
            "type": "object",
            "properties": {
                "command": {"type": "string", "description": "要执行的命令（自定义 shell-like 解析）"},
                "working_dir": {"type": "string", "description": "工作目录（可选，默认用户目录）"},
                "timeout": {"type": "integer", "description": "单次命令超时（秒），0 表示使用配置值"},
