...
def pdf_merge(
    ...
    if not added:
        return _ec("SAVE_FAILED", "没有成功添加任何 PDF 文件")

    try:
        with open(out, "wb") as f:
            writer.write(f)
    except Exception as e:
        return _ec("SAVE_FAILED", f"写入失败: {e}")

    lines = [f"[SUCCESS]", f"  merged: {len(added)} files → {out}", f"  pages: {len(writer.pages)}"]
    if skipped:
        lines.append(f"  warnings: {len(skipped)} skipped")
        for s in skipped[:5]:
            lines.append(f"    - {s}")
    return "\n".join(lines)
...
def pdf_rotate(
    ...
    try:
        ...
        return f"[SUCCESS]\n  output: {out}\n  rotated: {len(rotated)}/{total} pages by {rotation}°"
    except Exception as e:
...
def pdf_stamp_text(
    ...
    try:
        ...
        return f"[SUCCESS]\n  output: {out}\n  stamped: page {page} at ({x}, {y})pt \"{text[:30]}{'...' if len(text)>30 else ''}\""
    except Exception as e:
...
def pdf_select_pages(
    ...
    try:
        ...
        return f"[SUCCESS]\n  output: {out}\n  selected: {len(selected)}/{total} pages ({', '.join(str(p) for p in selected[:20])}{'...' if len(selected)>20 else ''})"
    except Exception as e:
...
def pdf_delete_pages(
    ...
    try:
        ...
        return f"[SUCCESS]\n  output: {out}\n  deleted: {len(page_set)} pages, kept: {len(kept)} pages"
    except Exception as e:
...
def pdf_compress(
    ...
    if shutil_which("gs"):
        try:
            ...
            if r.returncode == 0:
                ...
                return f"[SUCCESS]\n  output: {out}\n  compressed: {orig_size:,} → {new_size:,} bytes ({ratio:.0f}% reduction)"
        except Exception:
    ...
    try:
        ...
        new_size = out.stat().st_size
        ratio = (1 - new_size / orig_size) * 100 if orig_size > 0 else 0
        return f"[SUCCESS]\n  output: {out}\n  compressed: {orig_size:,} → {new_size:,} bytes ({ratio:.0f}% reduction)\n  method: pypdf (for better compression install ghostscript)"
    except Exception as e:
...
def pdf_watermark(
    ...
    try:
        ...
        return f"[SUCCESS]\n  output: {out}\n  watermarked: \"{text}\" on {total} pages"
    except Exception as e:
...
def pdf_render_preview(
    ...
    try:
        ...
        return f"[SUCCESS]\n  preview: {len(created)} pages @ {dpi}dpi → {out_d}\n" + "\n".join(f"  - {c}" for c in created)
    except Exception as e:
...
def pdf_redact(
    ...
    try:
        ...
        return f"[SUCCESS]\n  output: {out}\n  redacted: {redacted_count} regions\n  " + " | ".join(details)
    except Exception as e:
